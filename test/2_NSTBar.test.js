...
contract('NSTBar', ([alice, bob, carol]) => {
    beforeEach(async () => {
        this.nst = await NST.new({ from: alice });
        this.bar = await NSTBar.new(this.nst.address, { from: alice });
        this.nst.mint(alice, '100', { from: alice });
        this.nst.mint(bob, '100', { from: alice });
        this.nst.mint(carol, '100', { from: alice });
    });
    ...
    it('should not allow enter if not enough approve', async () => {
        ...
        await this.nst.approve(this.bar.address, '50', { from: alice });
        await expectRevert(
            this.bar.enter('100', { from: alice }),
            'ERC20: transfer amount exceeds allowance',
        );
        await this.nst.approve(this.bar.address, '100', { from: alice });
    ...
    it('should work with more than one participant', async () => {
        await this.nst.approve(this.bar.address, '100', { from: alice });
        await this.nst.approve(this.bar.address, '100', { from: bob });
        ...
        assert.equal((await this.nst.balanceOf(this.bar.address)).valueOf(), '30');
        // NSTBar get 20 more NSTs from an external source.
        await this.nst.transfer(this.bar.address, '20', { from: carol });
        // Alice deposits 10 more NSTs. She should receive 10*30/50 = 6 shares.
        ...
        assert.equal((await this.nst.balanceOf(this.bar.address)).valueOf(), '52');
        assert.equal((await this.nst.balanceOf(alice)).valueOf(), '70');
        assert.equal((await this.nst.balanceOf(bob)).valueOf(), '98');
    });
});
